...
export default function KanbanColumn({
  ...
  return (
    // Smooth transition with duration-500 and ease-in-out
    <div
      onClick={onColumnSelect}
      className={`flex flex-col bg-white rounded-xl shadow-md overflow-hidden transition-all duration-500 ease-in-out cursor-pointer border border-slate-200 ${
        isActive ? "col-span-8" : "col-span-2 hover:shadow-lg hover:border-slate-300"
      }`}
    >
      {/* Column Header */}
      <div
        className={`px-6 py-4 border-b border-slate-200 transition-all duration-500 ${
      ...
      >
        <div
          className={`flex items-center justify-between transition-all duration-500 ${
            isActive ? "flex-row" : "flex-col gap-2"
          }`}
        >
          <div className={`flex items-center gap-2 transition-all duration-500 ${isActive ? "flex-row" : "flex-col"}`}>
            ...
            <h2
              className={`font-semibold text-slate-900 transition-all duration-500 ${
                isActive ? "text-base whitespace-nowrap" : "text-xs whitespace-nowrap"
              }`}
              style={
                !isActive
                  ? {
      ...
      <div
        className={`flex-1 overflow-y-auto p-6 space-y-3 transition-opacity duration-500 ${
